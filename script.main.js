...
class User {
  ...
  buyTicket(ticket, quantity) {
    ...
    if (ticket.sellTickets(quantity)) {
      this.tickets.push({ event: ticket.event.name, category: ticket.category, quantity });
...
function initializeTicketPurchase() {
  ...
  buyButtons.forEach(button => {
    button.addEventListener('click', function(event) {
      ...
      const selectedTicket = eventTickets.find(ticket => ticket.event.name === eventName && ticket.category === category);
      ...
      if (selectedTicket) {
        ...
        if (currentUser.buyTicket(selectedTicket, quantity)) {
          ...
          alert(`Congratulations! You've successfully purchased a ${category} ticket for ${eventName}.`);
...
`
          ...
          // Optionally update UI or perform other actions upon successful purchase
          window.location.href = 'pay.html';
        } else {
          // Failed to buy tickets (insufficient quantity, etc.)
          alert(`Sorry, we couldn't process your request to purchase a ${category} ticket for ${eventName}.`);
        }
      } else {
        // Ticket not found (should not happen in this example)
...
// Call the function to initialize ticket purchasing
